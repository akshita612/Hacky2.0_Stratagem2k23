...
app.secret_key = 'al0612'

if __name__ == '__main__':  

    # Configure arguments
    ap = argparse.ArgumentParser()
    ap.add_argument("-u", "root", required=True, help="MySQL database user")
    ap.add_argument("-p", "AKSHITALITTLE", required=True, help="MySQL database password")
    args = vars(ap.parse_args())

    # Database credentials
    db_user = args['root']
    password = args['AKSHITALITTLE']
    database =  "master"
    host = "127.0.0.1"
    ...
    def login():
        if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
            ...
            cursor = cnx.cursor()
            cursor.execute(f'SELECT * FROM accounts WHERE username = "{root}" AND password = "{AKSHITALITTLE}"')
